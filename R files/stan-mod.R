...
#ggplot
#at t=200, with births & deaths corrected in the model
...
ggplot(data=df, aes(x=pars$age, y=x)) + 
  geom_line() + 
  geom_point(data=df, mapping=aes(x=pars$age, y=pars$Na), size=.2, col = "grey") + 
  scale_x_continuous(breaks=c(0,100, seq(0, 100, 20)), limits=c(0,100), expand=c(0,0)) + 
...
#lognormal(log(.05), .1)
hist(exp(rnorm(1000, log(.05), .3)), xlim=c(0,0.1))
